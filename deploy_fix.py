...
stdin, stdout, stderr = ssh.exec_command(
    'cd /opt/trans-app && docker compose -f docker-compose.prod.yml build bot --no-cache 2>&1 | tail -6'
)
...
print(stdout2.read().decode('utf-8', errors='replace').strip()[:300])
...
stdin4, stdout4, stderr4 = ssh.exec_command(
    'cd /opt/trans-app && docker compose -f docker-compose.prod.yml logs --tail=10 bot 2>&1'
)
